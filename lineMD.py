#!/bin/env python
# lineMD.py
# lineMD by Charles Yuan, 2015 (linear version)
...
def main():
    ...
    log(BLUE + "Simulation will end when ligand has passed " + MAGENTA + "%.*f" % (args.precision, args.max) + BLUE +
        " angstroms.\n" + END)
    log(BLUE + "Runs past " + MAGENTA + "%.*f" % (args.precision, args.min) + BLUE +
...
def parse():
    ...
    parser.add_argument("--max", help="magnitude, in angstroms, of distance the ligand must travel",
                        action="store", type=float)
    parser.add_argument("--min", help="distance allowed for ligand to approach the protein "
                                      "(traveling in incorrect direction)", action="store", type=float)
    ...
    parser.add_argument("--process_threads", help=SUPPRESS, type=int, action="store", default=cpu_count())
    ...
    parser.add_argument("--precision", help="number of decimal places used in calculations. "
                                            "Increase to allow more splits.", type=int, action="store", default=6)
...
def prep():
    ...
    if RUNANALYSIS and args.max is None:
        fail(RED + UNDERLINE + "Error:" + END + RED + " Please provide the maximum ligand travel distance.\n" + END)
    if RUNANALYSIS and args.min is None:
        fail(RED + UNDERLINE + "Error:" + END + RED + " Please provide the minimum ligand travel distance.\n" + END)
...
class Run(object):
    ...
    def processDist(self):
        ...
        Returns (dist, frame) of the largest dist
        """
        global PROTEINATOM
        global LIGANDATOM
        ...
        if self._dist == 0 or self._frame == 0:
            ...
            if self._clusterID == '0_0' and self._ID == 0:  # This is initial, skip all that stuff
                ...
                with directory(self.path):
                    ...
                    distances = parMap(getDist, sampleFrames, n=args.process_threads)

                    # Get the right frame with max ending distance
                    maxDistFrame, maxDist = max(distances, key=itemgetter(1))
                    os.rename("frame_%i.rst" % maxDistFrame, "end.rst")
                    compress("end.rst")

                    # recreate the coordinate file
                    with open("ptraj.in", 'w') as script:
                        script.write("parm %s\n" % PRMTOPPATH)
                        script.write("trajin coord.nc 1 %i 1\n" % int(maxDistFrame / int(args.steps / args.sample)))
...
class Cluster(object):
    """A Cluster object holds runs in a dictionary with ID keys. It knows its outermost distance and can calculate
    ...
    _dist = 0  # outermost distance this Cluster holds
    ...
    def readExplored(self, exploredDist, start=0):
        """Read the explored count, given an ascending-sorted exploredDist based on the explored_dist file.
        Returns an integer that can be used as start to shorten the array loop."""
        exploredCount = 0
        leftBound = self.dist - BINWIDTH
        for index, dist in enumerate(exploredDist[start:]):
            if dist > self.dist:  # We have passed this bin
                self._explored = exploredCount
                return index
            if dist >= leftBound:  # It is inside this bin, counting the left bound
                exploredCount += 1
        self._explored = exploredCount
        return 0  # failsafe
    ...
    def writeInfo(self):
        ...
        with open(self.path + "/cluster_info", 'w') as clusterInfo:
            clusterInfo.write("""MaxDistance: %.*f
...
def calcInitDist():
    ...
    if not RUNANALYSIS:
        ...
        if args.max is not None:
            ...
            log(BLUE + "Runs ending before " + MAGENTA + "%.*f" % (args.precision, initDist - args.min) +
...
def determineAtoms():
    """Find the appropriate ligand and protein center atoms. Can only be executed if the initial run has
     been prepared."""
    global LIGANDATOM
    global PROTEINATOM
...
def analysis(runDirectories):
    ...
    desiredCluster = round(args.max / BINWIDTH)
    ...
    for runID in runDirectories:  # should already be sorted
        ...
        if not found:
            ...
            if dist < initLeft:  # Left of initial cluster
                if dist < initLeft - args.min:  # Left of cutoff
...
def exportRestarts(runList):
    """Export the restart files for the completed trajectory and execute them."""
    distances = []
    for path in runList:
        info = []
    ...
    relativeFrequencies = histogram(distances, width)
    maxFreq = max(relativeFrequencies.values())

    global RESTARTPATH
    with directory(RESTARTPATH):
        system("rm %s/*" % RESTARTPATH)
        for index in xrange(1, len(runList)):
            path = runList[index]
            if not os.path.isdir('R' + str(index)):
                os.mkdir('R' + str(index))
                if index == 1:
                    system("touch R1/finished")
            with directory('R' + str(index)):
                if os.path.isfile(path + "/begin.rst"):
                    compress(path + "/begin.rst")
                shutil.copy(path + "/begin.rst.gz", "begin.rst.gz")
                shutil.copy(path + "/line.in", "line.in")
                shutil.copy(path + "/run_info", "run_info")
                ...
                with open("line.in") as inputFile:
                    for line in inputFile:
                        ...
                        if "ig = " in line:
                            ig = int(line.split("ig = ")[-1].split(',')[0])
                info = []
                ...
                if initial:
                    if os.path.isfile(path + "/end.rst"):
                        compress(path + "/end.rst")
                    shutil.copy(path + "/end.rst.gz", "end.rst.gz")
                    if os.path.isfile(path + "/coord.nc"):
                        compress(path + "/coord.nc")
                    shutil.copy(path + "/coord.nc.gz", "coord.nc.gz")
                else:  # Write all the new stuff
                    with directory("%s/R%i" % (RESTARTPATH, index)):
                        if args.queue_name in ["dept_gpu", "any_gpu", "bahar_gpu"]:
                            with open("run.sh", 'w') as runScript:
                                runScript.write("#!/bin/bash\n")
                                runScript.write("sleep 0.5; qsub -d . -q %s -S /bin/bash -N lineMD_R%i -l "
                                                "nodes=1:ppn=1:gpus=1 %s/R%i/qscript\n" % (args.queue_name, index,
                                                                                           RESTARTPATH, index))
                            with open("qscript", 'w') as qscript:
                                qscript.write("""#!/bin/bash
AMBERHOME=/usr/local/amber14
PATH=/usr/local/amber14/bin:$PATH
gunzip begin.rst.gz >> out 2>&1
pmemd.cuda -O -i line.in -o line.out -p %s -c begin.rst -r end.rst -x coord.nc
gzip line.out >> out 2>&1
rm mdinfo >> out 2>&1
touch finished >> out 2>&1
$cmd
""" % PRMTOPPATH)
                            ...
                            with open("run.sh", 'w') as runScript:
                                runScript.write("#!/bin/bash\n")
                                runScript.write("sleep 0.5; qsub -d . -q gpu_short -S /bin/bash -N lineMD_R%i -l "
                                                "nodes=1:ppn=1:gpus=1 -l feature=titan -l walltime=23:59:59 "
                                                "%s/R%i/qscript" % (index, RESTARTPATH, index))
                            with open("qscript", 'w') as qscript:
                                qscript.write("""#!/bin/bash
module purge
module load intel/2013.0
...
""" % PRMTOPPATH)
                        ...
                        with open(path + "/run_info") as runInfo:
                            for line in runInfo:
                                info.append(line.split()[1])
                            dist = float(info[1])
                        for key in relativeFrequencies.keys():
                            if key - width < dist < key:
                                thisFreq = relativeFrequencies[key]
                                break
                        if thisFreq == 0:
                            thisFreq = 1
                        samplingDegree = maxFreq / thisFreq  # For well-sampled locations, close to 1

                        def factors(n):
                            """Returns the list of factors of n, excluding 1 and n itself."""
                            return list(set(
                                reduce(list.__add__,
                                       ([i, n // i] for i in range(1, int(n ** 0.5) + 1) if n % i == 0))))[1:-1]

                        closestFactor = min(factors(ntwx), key=lambda x: abs(x - samplingDegree))
                        ntwx /= closestFactor
 ...
 ntp = 1, pres0 = 1.0, taup = 5.0,
/
""" % (endFrame, ntwx, endFrame, ig))

                    system("./run.sh >> out 2>&1")
                    log(GREEN + "Executed run %i on %s.\n" % (index, strftime("%c")))
...
=======
    global CLUSTERS
    global WORKDIR
>>>>>>> parent of 9258efa... some bug fixes
    global RUNNING
    global THREADS
